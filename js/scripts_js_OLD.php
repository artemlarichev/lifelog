...
/*	  if( amount<cnt )
  		{
  			...
  			$('#count_'+num).val($('#amount_'+num).val());
  			$('#count_'+num).focus();
...
for (var num = 1; num < p_count_mp+1; num++)
 {
  if( $('#count_mp_'+num).val()>0 )
      {
/*      amount=$('#amount_mp_'+num).val()*1;
      cnt =  $('#count_mp_'+num).val()*1
      if( amount<cnt )
          {
              alert('Количество заказа больше наличия!');
              $('#count_mp_'+num).val($('#amount_mp_'+num).val());
              $('#count_mp_'+num).focus();
          $.unblockUI();  return false;

          }
 */
           if($('#add1_mp_'+num).is(':checked')) {add1=1;}else{ add1='';}
           if($('#add2_mp_'+num).is(':checked')) {add2=1;}else{ add2='';}
           if($('#add3_mp_'+num).is(':checked')) {add3=1;}else{ add3='';}
           if($('#add4_mp_'+num).is(':checked')) {add4=1;}else{ add4='';}
           if($('#add5_mp_'+num).is(':checked')) {add5=1;}else{ add5='';}

                gropData[num] = {
                "id":$('#id_mp_'+num).val(),
                "type":'2_mp',
                "add1":add1,"add2":add2,"add3":add3,"add4":add4,"add5":add5,
                "count":$('#count_mp_'+num).val()
             };
           $('#count_mp_'+num).val('');
     }
 }
...
for (var num = 1; num < p_count2+1; num++)
 {
  if( $('#2count_'+num).val()>0 )
      {
      amount=$('#2amount_'+num).val()*1;
      cnt =  $('#2count_'+num).val()*1
/*      if( amount<cnt )
          {
              alert('Количество заказа больше наличия!');
              $('2#count_'+num).val($('2#amount_'+num).val());
              $('2#count_'+num).focus();
          $.unblockUI();  return false;
