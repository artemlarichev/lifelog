...
function Show_edit_block_basket(num,s_idx)
{       
   $.post("/catalog/get_session_data/", {"s_idx":s_idx}, function(arr){
      if(arr) {
         popup_size();
         $('.order_box_w').fadeIn();
         //$('.hide_element').show(); 

         $('#obf_title').text(arr['DetailNum']+' '+arr['MakeName']+' '+arr['PartNameRus']);
         $('#obf_count').val(arr['bascet_count']); 
         $('#part_price').val(arr['price_end']);
         sum = arr['bascet_count']*arr['price_end'];
         $('#obf_summ').text(sum.toFixed(2));   
         $('#OnlyThisBrand').attr('checked', arr['add2']);
         $('#bitAgree').attr('checked', arr['add4']);
         $('#bitWait').attr('checked', arr['add5']);
         $('#part_id').val($('#id_'+num).val());
         $('#part_num').val(num);
      }
   }, "json");
} 

function refresh_edit_box_price()
{
   var price = $('#part_price').val()*$('#obf_count').val();
   $('#obf_summ').html(price.toFixed(2));
}
...
function update_basket()
{
   ...
   if($('#bitAgree').is(':checked')) {add4=1;} else { add4='';}
   if($('#bitWait').is(':checked')) {add5=1;} else { add5='';}
   ...
   $.post(" /catalog/update_basket/",{data: $.toJSON(gropData) }, function(data){
      $("#basket_block").html(data) ;
      var num = $('#part_num').val();
      $('#count_'+num).val(gropData[0]['count']);
      var suma = gropData[0]['count']*gropData[0]['price'];
      $('#suma_'+num).text(suma.toFixed(2));
      //RefreshBasket();
