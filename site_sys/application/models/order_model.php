...
class Order_model extends Model {
    ...
    function send_order_change($order_id) {
        ...
        $user = $query->row();
        if (!$user)
            return false; // получаем пользователя
        if ($user->send_mail < 1)
            return false; // получаем пользователя
    ...
    function basket_to_order($order_data, $order_details, $mp_data = array()) {
        ...
        foreach ($order_details as $detail) {// делим детали в заказы потипу
            if ($detail['type'] == '0_sklad')
                $sklad[] = $detail;
            if ($detail['type'] == '1_ukr')
                $ukr[] = $detail;
            if ($detail['type'] == '2_mp')
                $mp[] = $detail;
        }
    ...
    function send_order_mail($email, $data, $from_mail, $nacenka = 1) {
        ...
        foreach ($_SESSION['basket'] as $val) {
            ...
            if ($val['type'] == '0_sklad') {
                $cu_group = $val['group'];
                if (!($val['group_1'] == '')) {
                    $cu_group.=' >> ' . $val['group_1'];
                }
                if (!($val['group_2'] == '')) {
                    $cu_group.=' >> ' . $val['group_2'];
                }
                if (!($group == $cu_group)) {
                    $group = $cu_group;
                    $text.="<tr>
            ...
            if ($val['add1'] > 0) {
                $text.="Только этот артикул   ";
            }
            if ($val['add2'] > 0) {
                $text.="Только этот производитель   ";
            }
            if ($val['add3'] > 0) {
                $text.="Только это количество  ";
            }
            if ($val['add4'] > 0) {
                $text.="Возможно повышение стоимости  ";
            }
            if ($val['add5'] > 0) {
                $text.=" Могу ждать месяц  ";
            }
