<?php
    class Catalog extends Controller {
        ...
        {   
            ...
            if(!isset($_SESSION['basket']) or !isset($_SESSION['basket_data']) ) {$_SESSION['basket']=array();$_SESSION['basket_data']=array('count'=>0,'sum'=>'0','val'=>$_SESSION['valuta']);};
            parent::Controller();                  
                        ...
                        { 
                            ...
                            if($val['type']=='0_sklad'){
                                ...
                                if($val['hidden']!='0'){
                                    ...
                                    $_SESSION['basket'][(int)$val['id'].$val['type']]['price_end'] = $this->catalog_model->get_price($data,$_SESSION['valuta'],'hidden');
                                }else
                                {
                                    $_SESSION['basket'][(int)$val['id'].$val['type']]['price_end'] = $this->catalog_model->get_price($data,$_SESSION['valuta'],$this->conf['nacenka']);
                                }
                            }elseif($val['type']=='1_ukr')
                            {
                                ...
                                if($_SESSION['valuta']==USD) 
                                    $_SESSION['basket'][(int)$val['id'].$val['type']]['price_end']= $this->catalog_model->get_price_ukr($data,1,$this->conf['discont']);
                                else 
                                    $_SESSION['basket'][(int)$val['id'].$val['type']]['price_end']= $this->catalog_model->get_price_ukr($data,$this->conf['kurs'],$this->conf['discont']);
...
";
        ...
        function ajax_find_mp(){
            ...
            if($data['result']=='error'){ echo "<h2>У вас нет доступа к ээтому разделу!</h2>";return;}
